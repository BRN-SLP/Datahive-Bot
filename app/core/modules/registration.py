    ...
    async def process(self) -> bool:
        ...
        for attempt in range(max_attempts):
            try:
                ...
                # Ensure fresh API session for each attempt
                from app.api.client import DatahiveAPI
                ...
                error_str = str(error)
                # Rate limit - retry with proxy change
                if "429" in error_str or "rate limit" in error_str.lower():
                    logger.warning(f"Rate limit hit, changing proxy... (attempt {attempt + 1}/{max_attempts})", self.email)
