...
class MultiprocessFarmingManager:
    ...
    async def start_multiprocess_farming(self, accounts: List[dict]) -> None:
        ...
        if settings.check_uniqueness_of_proxies:
            if len(proxies) < len(accounts):
                logger.warning(
                    f"⚠️ PROXY WARNING: You have {len(accounts)} accounts but only {len(proxies)} proxies. "
                    f"With 'check_uniqueness_of_proxies: true', this may lead to multiple accounts using the same proxy "
                    f"or devices failing to create. Recommended: 1 proxy per account."
                )
