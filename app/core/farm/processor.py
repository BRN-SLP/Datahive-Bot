...
class FarmProcessor:
    ...
    async def set_delay_for_devices(self, devices: List[Device], batch_size: int = 1000):
        ...
        for device in devices:
            ...
            device.next_job_request_at = get_sleep_until(seconds=delay)
        
        for chunk in self._batched(devices, batch_size):
            await Device.bulk_update_devices(chunk, ['next_ping_at', 'next_job_request_at'])
    ...
    async def _get_ready_devices(prepared_devices: List[Device], process_id: int, limit: int = 200) -> List[Device]:
        ...
        for device in prepared_devices:
            # Device is ready if its scheduled time has passed
            ping_ready = False
            job_ready = False
            
            if device.next_ping_at:
                ping_ready = await verify_sleep(device.next_ping_at)
            
            if device.next_job_request_at:
                job_ready = await verify_sleep(device.next_job_request_at)
            
            # Only add if at least one task is ready (time has passed)
            if ping_ready or job_ready:
