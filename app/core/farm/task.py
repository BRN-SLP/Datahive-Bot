...
class FarmTask:
    ...
    def run_offscreen_like(self, html_text: str, rules: Dict[str, Any]):
        ...
        for field_def in fields_cfg:
            try:
                ...
                name = field_def.get('field_name', 'unknown_field')
    ...
    def run_yaml_rules_on_html(self):
        """Run standard YAML-based rules on HTML content"""
        if not self.task_yaml_rules:
            return {}
    ...
    def build_task_json_data(self, context: Optional[Dict[str, Any]] = None):
        ...
        if self.target_url_html is None:
            ...
            try:
                ...
                logger.error(f'Task {self.task_id} parsing error: {e}')
                step_outputs = {
                    'pageData': {
                        'fields': {
                            'title': '',
