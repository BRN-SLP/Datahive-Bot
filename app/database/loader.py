...
def load_accounts(file_name: str = "login_accounts.txt") -> List[dict]:
    """
...
def load_farm_accounts(file_name: str = "farm_accounts.txt") -> List[str]:
    """
...
def load_proxies() -> List[str]:
    # Use absolute path relative to project root
    project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    file_path = os.path.join(project_root, "config", "data", "proxies.txt")
    ...
    try:
        ...
        # Enforce uniqueness if requested (though we do it by default now for safety)
        unique_proxies = list(dict.fromkeys(proxies))
        if len(unique_proxies) < len(proxies):
            logger.debug(f"Removed {len(proxies) - len(unique_proxies)} duplicate proxies")
        
        logger.debug(f"Loaded {len(unique_proxies)} unique proxies")
