...
def get_imap_server_for_email(email: str) -> str:
    """Resolve IMAP server for email from config"""
    from app.config.settings import get_settings
    settings = get_settings()
    
    imap_config = settings.imap_settings
    
    # Check single IMAP setting
    if imap_config.get("use_single_imap", {}).get("enable", False):
        server = imap_config.get("use_single_imap", {}).get("imap_server", "")
        if server:
            return server
